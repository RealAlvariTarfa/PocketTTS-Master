...
This repository can be deployed to Firebase via Cloud Run, exposing the built HTTP server as a Firebase-hosted API under `/api/**`.
...
3. Add your `models/` and `voices/` directories to the repository before deploying. The runtime expects the model files under `models/` and voice samples under `voices/`.

4. Deploy Cloud Run and Hosting together:

```bash
firebase deploy --only hosting,run
```

5. After deploy, API calls can be made to the Firebase Hosting endpoint. For example:

```bash
curl -X POST https://<YOUR_PROJECT_ID>.web.app/api/v1/audio/speech \
  -H "Content-Type: application/json" \
  -d '{"model":"tts-1","input":"Hello world!","voice":"voice","response_format":"wav"}' \
  --output speech.wav
```

If you want to deploy the service manually with Cloud Run instead, you can build and deploy the Docker container directly:

```bash
docker build -t pockettts-api .
gcloud run deploy pockettts-api --source . --region us-central1 --allow-unauthenticated
```
